...
export class MainScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Set up camera world bounds and initial position
    const camera = this.cameras.main;
    camera.setBounds(0, 0, WORLD_WIDTH, WORLD_HEIGHT);
    // Center camera on the Q1 starting area (where buildings are placed at y=32-38)
    const startPos = this.gridToScreen(20, 32);
    camera.centerOn(startPos.x, startPos.y);
    camera.setZoom(0.6); // Start slightly zoomed out
